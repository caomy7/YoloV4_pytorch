...
class DarkNet(nn.Module):
    def __init__(self, layers):
        ...
        self.inplanes = 32
        self.convbn1 = conv_bn(3, self.inplanes, kernel=3,
                               stride=1, padding=1, activate='leaky')
    ...
    def _make_layer(self, planes, blocks):
        layers = []
        #  downsample
        layers.append(('residual_downsample', conv_bn(
            self.inplanes, planes[1], kernel=3, stride=2, padding=1, activate='leaky')))
        self.inplanes = planes[1]
        for i in range(0, blocks):
            layers.append(("residual_{}".format(
                i), DarknetBlock(self.inplanes, planes)))
